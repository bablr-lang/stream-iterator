...
let conrolPromises = (globalThis.__stream_generator_waitForPromises__ ||= new WeakMap());
...
export const wait = (promise) => {
  conrolPromises.set(promise, true);
  return promise;
};

export const continue_ = () => {
  let sigilPromise = new Promise(() => {});
  conrolPromises.set(sigilPromise, false);
  return sigilPromise;
};
...
export class AsyncGenerator {
  ...
  next(value) {
    ...
    let cb = (step) => {
      ...
      if (conrolPromises.has(step.value)) {
        conrolPromises.delete(step.value);
...
export class StreamGenerator {
  ...
  next(value) {
    ...
    let cb = (step) => {
      ...
      if (conrolPromises.has(step.value)) {
        let shouldWait = conrolPromises.get(step.value);
        conrolPromises.delete(step.value);
